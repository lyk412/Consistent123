...
class CLIP(nn.Module):
    ...
    def train_step(self, clip_z, pred_rgb, grad_scale=10, adaptive=False, **kwargs):
        """
            Args:
                grad_scale: scalar or 1-tensor of size [B], i.e. 1 grad_scale per batch item. 
                adaptive: if true, img-img cal is not needed for checking clip loss
        """
        ...
        if adaptive:
            return ((image_z * clip_z['text']).sum(-1) * grad_scale).mean()
        else:
            return loss
