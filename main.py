...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_mesh_normal', type=float, default=100, help="loss scale for mesh normal smoothness") # 0.5
    parser.add_argument('--lambda_mesh_laplacian', type=float, default=100, help="loss scale for mesh laplacian") # 0.5
    ...
    parser.add_argument('--lambda_2d_normal_smooth', type=float, default=10, help="loss scale for 2D normal image smoothness") # 0 
    ...
    parser.add_argument('--save_guidance_interval', type=int, default=5, help="save guidance every X step")
    ...
    parser.add_argument('--zero123_ckpt', type=str, default='/home/data/lyk/dreamfusion/pretrained/zero123/zero123-xl.ckpt', help="ckpt for zero123")
    ...
    # for consistent123
    parser.add_argument('--weight_method', type=str, default=None, choices=['linear', 'exponential', 'log'],
                        help="Choose the weight adjustment method. Options: 'linear', 'exponential', 'log'")
    parser.add_argument('--render_interval', type=int, default=-1, help="number of iterations for nerf rendering and save images, and also number of iterations for check CLIP loss")
    parser.add_argument('--least_3Donly', type=int, default=None, help="use clip check if set, at least number of iterations for only 3D guidance")
    parser.add_argument('--most_3Donly', type=int, default=None, help="use clip check if set, at most number of iterations for only 3D guidance")
    parser.add_argument('--threshold', type=float, default=None, help="threshold of changing rate for checking converge")
    parser.add_argument('--last_N', type=int, default=None, help="used to compute_average_change_rates")
    parser.add_argument('--convergence_path', type=str, default=None, help="convergence_path")
    parser.add_argument('--nerf_iters', type=int, default=None, help="number of iterations for nerf stage, must given in the DMTet training")
    parser.add_argument('--dmtet_iters', type=int, default=None, help="number of iterations for dmtet stage, must given in the Nerf training")
    
    parser.add_argument('--lr_scale_nerf', type=float, default=1, help="max learning rate")
    # same function, but only2D_iters_to_end is prior when both are given
    parser.add_argument('--ratio_to_only2D', type=float,
                    default=None, help="under the clip(3D + weight) mode and when dmtet, when SD loss weight achieve ratio_to_only2D, then SD(2D) only")
    parser.add_argument('--only2D_iters_to_end', type=int,
                    default=None, help="under the clip(3D + weight) mode and when dmtet, when iters achieve (total - only2D_iters_to_end), then SD(2D) only")
    
    # some setting of ablation
    parser.add_argument('-all_weight', action='store_true', help="use weight method for two stages all the time if True")
    parser.add_argument('--sigma', type=float, default=1.0,
                    help="for all_weight. e-(t / sigma * T), the 3D weight fall slower when sigma is large")
    parser.add_argument('-all_3D', action='store_true', help="use only 3D guidance for two stages all the time if True")
    parser.add_argument('-batch_test', action='store_true', help="batch test first 3D + second weight")
    parser.add_argument('-binary2D', action='store_true', help="the weight of 2D prior ture to 1 from 0 and keep util ending")
    parser.add_argument('--fix3D_iters', type=int, default=-1, help="fix steps for 3D and turn to dynamic prior")
    parser.add_argument('-only3D_only2D', action='store_true', help="first stage only 3D and second stage only 2D")
    parser.add_argument('-all3D_binary2D', action='store_true', help="both two stages use 3D and second stage introduces 2D")

    parser.add_argument('--write_video', action='store_true', help="write video when test if write_video==True else write images")

    # --- very important: Nerf training must be complete ---
    opt = parser.parse_args()

    if opt.render_interval % opt.known_view_interval == 0:
        print(f'\nerror: render_interval % known_view_interval == 0')
        exit()
    ...
    if opt.image is not None or opt.image_config is not None:
        ...
        if opt.text is None:
            ...
            #     opt.t_range = [0.2, 0.6] #t_range是啥
            #     opt.known_view_interval = 2
            #     opt.lambda_3d_normal_smooth = 20
            # opt.bg_radius = -1
            
           
            if opt.weight_method is not None:
                
                print(f"use {opt.weight_method}, ['SD', 'zero123']")
                ...
                if opt.only3D_only2D:
                    opt.guidance = ['SD', 'zero123']
                    
                if opt.all3D_binary2D:
                    opt.guidance = ['SD', 'zero123']
                    

            if  opt.least_3Donly is not None:
                # least_3Donly: 3Donly + weight method and the change time is case-aware
                # clip for case-aware
                print(f"clip for case-aware use ['SD', 'zero123', 'clip'] ")
                opt.guidance = ['SD', 'zero123', 'clip'] 
        ...
        print(f"use guidance: {opt.guidance}")
    ...
    # 去除不可见的空格 制表符等
    cleaned_init_with = opt.init_with.strip()
    opt.init_with = cleaned_init_with
    
    if opt.dmtet and opt.init_with != '':
        if opt.init_with.endswith('.pth'):
            # load pretrained weights to init dmtet
            # opt.init_with = os.path.join('/home/data/lyk/dreamfusion/trials_linear', opt.init_with)
            ...
            print(f'init with {opt.init_with}')
    ...
    if opt.six_views:
        ...
        if opt.gui:
            ...
            trainer.test(test_loader, write_video=opt.write_video)
